...
def get_running_time_of_fms_algorithms(
    ...
    This algorithm calculate the time of both algorithm applyed each to arrays
    of size {1, 2, ..., n}. Return a dict mapping each running time for
    recursive / iterative algorithm.
    """
    random_arrays = []
    for i in range(1, n + 1):
        array = [random.randrange(-50, 50) for _ in range(1, i + 1)]
...
if __name__ == "__main__":
    fig = plot_comparison(number_of_arrays=40)
    fig.show()
